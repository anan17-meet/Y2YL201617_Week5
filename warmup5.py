...
and a list of integers movie_lengths (in minutes) and returns a boolean indicating whether there
...
'''
flight_length = 250
movie_lengths = [90,145,185,125,125]

def in_flight_movies(flight_length, movie_lengths):
  for movie1 in range(len(movie_lengths)):
  	for movie2 in range(movie1+1,len(movie_lengths)):
  		if movie_lengths[movie2]+movie_lengths[movie1]==flight_length:
  			print("if2")
  			return True
  return False

print(in_flight_movies(flight_length,movie_lengths))
